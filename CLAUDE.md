...
- **Postgres identity sequences:** CSV rows include explicit primary keys, which does **not** advance PostgreSQL identity/serial sequences. **`Program.cs` runs `CsvDataSeeder.ResyncPostgresIdentitySequencesAsync` on every startup** (Npgsql only) so the next ORM insert (e.g. `POST /api/donations`, new `supporters`) gets a fresh ID instead of colliding with seeded data.
